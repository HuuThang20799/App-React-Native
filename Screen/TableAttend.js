...
function TableAttend({navigation,route}){
  class  DataTableAttend extends Component{
    ...
    componentDidMount(){
      ...
      fetch("http://192.168.1.7/SystemAptech/DiemDanhSV/API_DiemDanh/API_ShowStudent.php",{
        method: 'POST',
        headers: { 
    ...
    getValueChecked(){
      ...
      for(let j=0;j<Selected.length;j++){
        ...
        fetch("http://192.168.1.7/SystemAptech/DiemDanhSV/API_DiemDanh/API_XuLyDiemDanh.php",{
            method: 'POST',
            headers: { 
            //'Content-Type': 'application/json'
            },
          ...
          <View style={{backgroundColor:'white',width:'100%',padding:10}}>          
            ...
            <ScrollView>
              ...
              <View style={{flexDirection:'row',borderColor:'black',borderWidth:2}}>
                    <Text style={{width:80,borderColor:'#00FA9A',borderWidth:2,textAlign:'center',fontSize:20,backgroundColor:'black',color:'#00FA9A'}}>MSSV</Text>
                    <Text style={{width:230,borderColor:'#00FA9A',borderWidth:2,textAlign:'center',fontSize:20,backgroundColor:'black',color:'#00FA9A'}}>HỌ TÊN</Text> 
                    <Text style={{width:60,borderColor:'#00FA9A',borderWidth:2,textAlign:'center',fontSize:20,backgroundColor:'black',color:'#00FA9A'}}>DD</Text> 
              </View>
                ...
                renderItem={({item,index}) =>(      
                  <View style={{flex:1,flexDirection:'row',borderColor:'black',borderWidth:2}}>                   
                    <Text style={{width:80,borderColor:'#00FA9A',borderWidth:2,textAlign:'center',fontSize:18}}>
                        {item.SV_MSSV}</Text> 
                    <Text style={{width:230,borderColor:'#00FA9A',borderWidth:2,fontSize:18,textAlign:'left'}}>
                        <Text>  </Text>{item.SV_TEN}
                    </Text> 
                    <View style={{width:60,borderColor:'#00FA9A',borderWidth:2,fontSize:18,alignItems:'center'}}>
