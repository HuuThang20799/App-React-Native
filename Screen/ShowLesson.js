...
function ShowLessonList({navigation,route}){
  ...
  async function TaoBuoiHoc(){
    ...
    if(txtLessonName == 'FirstCreateNameLesson'){
      ...
      let result= await fetch("http://192.168.1.7/SystemAptech/DiemDanhSV/API_DiemDanh/API_CreateLesson.php",{
          method: 'POST',
          headers: {},
          body: data 
      });
  ...
  class  DataTableAttend extends Component{
    ...
    componentDidMount(){
      ...
      fetch("http://192.168.1.7/SystemAptech/DiemDanhSV/API_DiemDanh/API_ShowLesson.php",{
        method: 'POST',
        headers: { 
    ...
    render() {
      ...
      return(
          <View style={{backgroundColor:'white',padding:2,alignItems:'center',textAlign:'center',width:'99%'}}>
            <ScrollView horizontal={true}>
              <ScrollView>
                <FlatList  style={{ flex:1,paddingBottom: 600,paddingTop:10}}
                data={data}
                renderItem={({item}) =>(      
                  <View style={{flex:1,flexDirection:'row',alignItems:'center'}}>
                    
                    <View style={{flex:1,alignItems:'center',borderColor:'aqua',borderWidth:0}}>
