...
function EditAttendsFunction({navigation,route}){
  class  ClassComponentEditAttend extends Component{
    ...
    componentDidMount(){
      ...
      fetch("http://192.168.1.7/SystemAptech/DiemDanhSV/API_DiemDanh/API_ShowResultAttend.php",{
        method: 'POST',
        headers: { 
    ...
    getValueChecked(){
      ...
      for(let j=0;j<Selected.length;j++){
        ...
        fetch("http://192.168.1.7/SystemAptech/DiemDanhSV/API_DiemDanh/API_XuLyThayDoiDiemDanh.php",{
            method: 'POST',
            headers: { 
              //'Content-Type': 'application/json'
            },
          ...
          <View style={{backgroundColor:'white',width:'100%',padding:5}}>
            ...
            <ScrollView>
              ...
              <View style={{flexDirection:'row',borderColor:'black',borderWidth:2}}>
                    <Text style={{width:80,borderColor:'#00FA9A',borderWidth:2,textAlign:'center',fontSize:18,backgroundColor:'gray',color:'black',fontWeight:'bold'}}>MSSV</Text>
                    <Text style={{width:200,borderColor:'#00FA9A',borderWidth:2,textAlign:'center',fontSize:18,backgroundColor:'gray',color:'black',fontWeight:'bold'}}>HỌ TÊN</Text> 
                    <Text style={{width:50,borderColor:'#00FA9A',borderWidth:2,textAlign:'center',fontSize:18,backgroundColor:'gray',color:'black',fontWeight:'bold'}}>KQ</Text>
                    <Text style={{width:50,borderColor:'#00FA9A',borderWidth:2,textAlign:'center',fontSize:18,backgroundColor:'gray',color:'black',fontWeight:'bold'}}>EDIT</Text> 
              </View>
              <FlatList  style={{ flex:1,width:'100%'}}
                ...
                renderItem={({item,index}) =>(    
                  ...
                  <View style={{flex:1,flexDirection:'row',borderColor:'black',borderWidth:2}}>                   
                    <Text style={{width:80,borderColor:'#00FA9A',borderWidth:2,textAlign:'center',fontSize:18}}>
                        {item.SV_MSSV}</Text> 
                    <Text style={{width:200,borderColor:'#00FA9A',borderWidth:2,fontSize:18,textAlign:'left'}}>
                        <Text>  </Text>{item.SV_TEN} 
                    </Text> 
                    <Text style={{flex:1,width:50,borderColor:'#00FA9A',borderWidth:2,fontSize:18,alignItems:'center',textAlign:'center'}}>
                        {item.KET_QUA == 0 ? <AntDesign name="closecircleo" size={32} color="red" />: <Feather name="check-circle" size={32} color="green" />}
                    </Text> 
                    <View style={{width:50,borderColor:'#00FA9A',borderWidth:2,fontSize:18,alignItems:'center'}}>
